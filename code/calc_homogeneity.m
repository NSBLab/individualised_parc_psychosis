function homogeneity = calc_homogeneity(setup,ROI_exc)
...
% set list of subjects that have a run 2
subj = [];
...
for s = 1:length(subj)
	    ...
	    img_lh = MRIread(append(run_02_surface,'/sub-',num2str(subj(s)),'/sub-',num2str(subj(s)),'_run-02_lh_medialwall_fillin.mgz'));
	    img_rh = MRIread(append(run_02_surface,'/sub-',num2str(subj(s)),'/sub-',num2str(subj(s)),'_run-02_rh_medialwall_fillin.mgz'));
...
% figure ; subplot(3,2,[2,4,6]) ; hold on ; raincloud_plot(hom_grp_pat, '#ff0202') ; raincloud_plot_3(hom_ind_pat, '#2c7bb6') ; raincloud_plot_1(hom_grp_con, '#ff9191') ; raincloud_plot_2(hom_ind_con, '#abd9e9'); xlabel('homogeneity');
