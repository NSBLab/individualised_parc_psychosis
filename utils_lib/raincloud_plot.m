...
% 'box' of 'boxplot'
h{2} = rectangle('Position',[Y(1) -2.2 Y(2)-Y(1) wdth]);
set(h{2},'EdgeColor',cl)
set(h{2},'LineWidth',2);
% could also set 'FaceColor' here as Micah does, but I prefer without

% mean line
h{3} = line([Y(3) Y(3)],[-2 -1.6],'col',cl ,'LineWidth',2);

% whiskers
h{4} = line([Y(2) Y(5)], [-1.8 -1.8],'col',cl,'LineWidth',2);
h{5} = line([Y(1) Y(4)],[-1.8 -1.8],'col',cl,'LineWidth',2);

% raindrops
h{3} = scatter(X,jit - 1.8);
h{3}.SizeData = 10;
