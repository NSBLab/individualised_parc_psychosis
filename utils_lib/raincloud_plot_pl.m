function h = raincloud_plot(X,cl,cl_edge,loc)
...
% 'box' of 'boxplot'
h{2} = rectangle('Position',[Y(1) -(loc+0.4) Y(2)-Y(1) wdth]);
set(h{2},'EdgeColor',cl_edge)
set(h{2},'LineWidth',2);
% could also set 'FaceColor' here as Micah does, but I prefer without

% mean line
h{3} = line([Y(3) Y(3)],[-(loc+0.2) -(loc-0.2)],'col',cl_edge ,'LineWidth',2);

% whiskers
h{4} = line([Y(2) Y(5)], [-loc -loc],'col',cl_edge,'LineWidth',2);
h{5} = line([Y(1) Y(4)],[-loc -loc],'col',cl_edge,'LineWidth',2);

% raindrops
h{3} = scatter(X,jit - loc);
h{3}.SizeData = 10;
h{3}.MarkerFaceColor = cl;
h{3}.MarkerEdgeColor = 'none';
h{3}.MarkerFaceAlpha = 0.35;
