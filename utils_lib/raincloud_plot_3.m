...
% 'box' of 'boxplot'
h{2} = rectangle('Position',[Y(1) -4.4 Y(2)-Y(1) wdth]);
set(h{2},'EdgeColor',cl)
set(h{2},'LineWidth',2);
% could also set 'FaceColor' here as Micah does, but I prefer without

% mean line
h{3} = line([Y(3) Y(3)],[-4.2 -3.8],'col',cl ,'LineWidth',2);

% whiskers
h{4} = line([Y(2) Y(5)], [-4 -4],'col',cl,'LineWidth',2);
h{5} = line([Y(1) Y(4)],[-4 -4],'col',cl,'LineWidth',2);
